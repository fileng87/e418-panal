...
function getClientIp(request: NextRequest): string | undefined {
    ...
    if (process.env.NODE_ENV !== 'production' && process.env.TEACHER_IPS) {
        ...
        if (firstTeacherIp) {
            console.warn(`Non-production mode: Returning first TEACHER_IP (${firstTeacherIp}) from list for testing. Remove before deployment!`);
            return firstTeacherIp;
        } else {
             console.error(`Non-production mode: TEACHER_IPS is set but seems empty or invalid.`);
        }
    }
    // --------------------

    console.warn('x-forwarded-for header not found and not in non-production mode with TEACHER_IPS set. Cannot determine client IP.');
...
export function middleware(request: NextRequest) {
    ...
    console.log(`Attempting request from IP: ${clientIp}, Path: ${pathname}, Allowed Teacher IPs: ${teacherIpsString}`);
    ...
    // 將字串分割為陣列，並去除空白
    const allowedTeacherIps = teacherIpsString.split(',').map(ip => ip.trim()).filter(ip => ip); // filter(ip => ip) 確保移除空字串

    if (allowedTeacherIps.length === 0) {
         console.error("TEACHER_IPS environment variable is set, but contains no valid IPs after splitting.");
         return new Response('Server configuration error: No valid teacher IPs configured', { status: 500 });
    }
    ...
    if (isTeacherIp) {
        ...
        // --- 非教師機 IP (或無法確定 IP) 邏輯 ---
        console.log(`Non-teacher IP detected or IP mismatch (Client: ${clientIp}).`);
        if (pathname === '/') {
            console.log("Allowing non-teacher access to /");
            return NextResponse.next(); 
        } else {
            console.log(`Blocking non-teacher access to ${pathname}, redirecting to /forbidden`);
            const url = request.nextUrl.clone();

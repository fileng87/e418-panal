...
export async function GET() {
    ...
    try {
        ...
        const enabledFilters = response.data.filters?.filter(f => f.enabled) ?? [];
        // 過濾掉空字串的自訂規則
        const rawUserRules = response.data.user_rules ?? [];
        const userRules = rawUserRules.filter(rule => rule && rule.trim() !== '');

        console.log(`Successfully fetched ${enabledFilters.length} enabled filters and ${userRules.length} non-empty user rules.`);
        ...
        if (axios.isAxiosError(error)) {
            let errorMessage = `請求 AdGuard API 失敗: ${error.message}`;
             if (error.response?.status === 401) errorMessage = "AdGuard Home 驗證失敗。";
             else if (error.response?.status) errorMessage = `AdGuard Home API 回應錯誤: ${error.response.status}`;
             else if (error.code === 'ECONNABORTED') errorMessage = "連線 AdGuard Home 超時。";
             return NextResponse.json({ message: errorMessage }, { status: error.response?.status || 500 });
        }
        const message = error instanceof Error ? error.message : String(error);
        return NextResponse.json({ message: `獲取 AdGuard 過濾器時發生內部錯誤: ${message}` }, { status: 500 });
    }
} 
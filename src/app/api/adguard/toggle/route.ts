...
export async function POST(request: Request) {
  ...
  if (!adguardUrl || !adguardUser || !adguardPass) {
    console.error("AdGuard Home 環境變數未設定 (ADGUARD_URL, ADGUARD_USERNAME, ADGUARD_PASSWORD)");
    return NextResponse.json({ message: 'AdGuard Home 設定不完整' }, { status: 500 });
  }
  ...
  try {
    ...
    if (typeof enable !== 'boolean') {
      return NextResponse.json({ message: '請求格式錯誤，缺少 enable 參數' }, { status: 400 });
    }

    const apiUrl = `${adguardUrl.replace(/\/$/, '')}/control/protection`;
    console.log(`Sending request to ${apiUrl} to ${enable ? 'enable' : 'disable'} protection.`);
    ...
    if (response.status === 200) {
      console.log(`Successfully ${enable ? 'enabled' : 'disabled'} AdGuard protection.`);
      return NextResponse.json({ message: `AdGuard 保護已成功 ${enable ? '開啟' : '關閉'}` });
    } else {
      // 理論上 axios 會對非 2xx 狀態拋出錯誤，但以防萬一
      console.error("Unexpected success status from AdGuard API:", response.status);
      return NextResponse.json({ message: 'AdGuard API 回應非預期成功狀態' }, { status: response.status });
    }
    ...
    return NextResponse.json({ message: `切換 AdGuard 保護時發生內部錯誤: ${message}` }, { status: 500 });
  }
} 